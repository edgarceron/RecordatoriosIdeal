<?php
class ReporteLlamadasAction extends CAction
{
    ...
    public function run()
    {
		...
		if(isset($_GET['recibida']) && $_GET['recibida'] != ""){
			$recibida = $_GET['recibida'];
			$atributos['recibida'] = $recibida;
		}
		else{
			$atributos['recibida'] = "";
		}
		...
		$criteria->select = "recibida";
		$criteria->addCondition('t.tipo = "CALL"');
		...
		if(isset($recibida)){
			$criteria->addCondition('t.recibida = "'. $recibida . '"');
		}
        ...
        $this->controller->render('reporteLlamadas',$atributos);
    }
}
?>
